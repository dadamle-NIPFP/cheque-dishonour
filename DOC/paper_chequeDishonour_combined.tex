...
\begin{document}
	...
	%SOMETHING TO BE ADDED LATER: Link number of cases to state GDP. No of cases is directly linked to the economic activity. If delay hinders performance, then it will hurt the well performing states more. state GDP will suffer and it will have asymmetrical impact on Indian GDP. Insert the figure of SGDP and no of NI 138 cases.
...
%		\includegraphics[width = 0.9\textwidth]{/SLIDES/IMAGES/surv_states.pdf}
...
%Given this background, the purpose of this paper is fourfold. First, using the case-level eCourts data, for the first time in India, we estimate the real judicial caseload due to the Negotiable Instruments Act, 1881 cases. Second, we estimate the average case disposition time and its variation over time and across states.  Third, we empirically test our conceptual framework of procedural determinants of case disposition time. In line with \cite{bielen2015}, using the interim and final orders data at the case level, we establish the link between the specific case characteristics and empirically examine the procedural issues and micro-level determinants of case duration, while controlling for characteristics that are inherent to courts and judges. Note that the empirical literature on the length of court proceedings currently focuses mainly on determinants at the country and court levels, e.g. public budgets for courts, backlogs, number of judges, etc. Some studies also focus on the case-level determinants like the number of parties involved, number of pleadings, etc. Given access to the interim and final orders data, we focus on the case characteristics that can be directly targetted using procedural reforms. 

%	Given access to the interim and final orders data, we focus on the reasons related to the procedural matters together with specific case characteristics that are relatively scarce, presumably as a consequence of lack of access to case-level data.

%And finally, we analyze the effect of the 2015 amendment of the NI Act, 1881 by comparing the duration of cases handled before and after their implementation using regression analysis.

%We also determine the number of hearing as an additional proxy for the trial length. 
...
%Some may be NI Act cases, even though the Act Name field does not say so.
...
%	\begin{enumerate}
	%		\item Our estimates suggest that cheque dishonor cases represent approximately 13\% of courts' workload (pending and disposed) in India. This number could be an underestimate because, in about 36\% of the cases in the sample, orders were either unavailable or could not be parsed. Some may be NI Act cases, even though the Act Name field does not say so.
	%		\item We find significant variation in case disposal time across states. Median disposal time varies from as low as 218 days in Karnataka (roughly in line with the court's recommendation) to as high as 547 days in Himachal Pradesh with a median disposal time of 395 days. We also find that only a fifth of the cases gets disposed of within the prescribed limit of six months.
	%		\item Controlling for state and year fixed effects, our findings suggest that cases where the accused fails to appear to take an additional 213 days and 7 hearings to dispose of, all else being equal. Similarly, cases run as summons trials take 111 days and 7.2 hearings more than summary trials. Cases with jurisdictional issues take 287 days and 5.7 hearings more to dispose of than cases without these issues. Cases with a multiplicity of proceedings take an additional 171 days and 10 hearings to dispose of than those without. Contested cases take fewer days to dispose of than uncontested cases, even though they take more hearings.
	%		\item Our results also indicate that 2015 amendments focused on clarifying the jurisdiction-related issues for filing cases and mandatory centralization of cases against the same drawer has led to a major decrease in case duration. Our estimate suggests that it has reduced the case duration on average by more than six months within a span of two years.
%	\end{enumerate}
%			
	% SOMETHING TO BE ADDED LATER: \textbf{How we contribute to the debate?}
			
	The rest of this paper is organized as follows: after this introduction, \cref{sec:history} elaborates on the history of cheque dishonor provisions in India and their relation with judicial delays. In \cref{sec:select-case-char}, based on our conversations with practicing lawyers, other stakeholders, and the recommendations of the Amici Curiae and the Supreme Court, we figure out the key case characteristics that are linked to the procedural delay and can be directly targeted by the court interventions. \Cref{sec:methodology} describes the methodology, and \cref{sec:results} presents the results thereof. In \cref{sec:2015amend}, we analyze the effectiveness of 2015 amendments by comparing the duration of cases handled before and after the procedural reforms were implemented. Finally, \cref{sec:conclusion} concludes the paper and presents the way forward.
	
	%	The rest of this paper is organized as follows: after this introduction, \cref{sec: history} elaborates on the history of cheque dishonor provisions in India and their relation with judicial delays. In \cref{sec:select-case-char}, based on our conversations with practicing lawyers, other stakeholders, and the recommendations of the Amici Curiae and the Supreme Court, we figure out the key procedural reforms and case-specific characteristics that can be targeted by direct court interventions. \Cref{sec:methodology} describes the methodology, and \cref{sec:results} presents the results thereof. Finally, \cref{sec: conclusion} concludes the paper and presents the way forward.
	...
	% SOMETHING TO BE DONE LATER: If the 1988 reform made cheque bouncing as a criminal offence, then this must be mentioned in the previous paragraph. Some literature must be cited on people's view on cheque bouncing cases being classified as a criminal offence.
	...
	%Accordingly, we chose to examine the effect of six case characteristics that map onto the five intervention targets mentioned above. 
	\begin{enumerate}
	...
	\end{table}
		...
		%	\begin{enumerate}
			...
			%		\item In the case of Puducherry, we don't find any interim or final order for the NI cases.
			%		\item Given our limitation to process information recorded in regional language, we dropped states where less than 2\% of the final orders (as a proportion of total NI cases) were in English (Daman and Diu, Uttarakhand, Uttar Pradesh, Rajasthan, Tripura, Chhattisgarh, Jammu and Kashmir, and Bihar dropped)
			%		\item We also drop states where NI cases in which interim or final order is available in English to represent less than 1\% of the total cases (Telangana, Jharkhand, Kerala, Tamil Nadu, Assam, Orissa, West Bengal, and Madhya Pradesh dropped)
			%		\item So we were left with 2 UTs - Chandigarh and Delhi, 2 small states - Goa and Himachal Pradesh, 2 mid-size states - Punjab and Haryana, 4 large states - Karnataka, Gujarat, Maharashtra, and Andhra Pradesh. The NI cases as a proportion of total cases vary from 7.35\% in Himachal Pradesh to 22.06\% in Chandigarh in our final data. The cases where the final order is available in English as a proportion of total NI cases vary from 23.11\% in Gujarat to 80\% in Chandigarh. In terms of data quality, Chandigarh, Punjab, Haryana, and Karnataka tops the chart. 
			%	\end{enumerate}
		...
		%to ensure that the final dataset is well representative of the  
		
		%We identified the states to exclude from the study based on the following criteria:
		
		%	\begin{itemize}
			%		\item states and Union Territories where we were unable to download sample data (this is because of the data quality of the e-courts database);
		...
		%For details concerning the selection, see appendix \ref{sec:sample_selection}.
...
%	\end{longtable}
	...
	%Our objective is two-fold, to examine whether the interventions proposed by the Amicus Curiae are likely to affect the duration of cheque-dishonor cases, and if so, what is the magnitude of the effect. To that end, we first identified cases with specific characteristics that are thought to increase the duration of a cheque-dishonor case. 
	...
	\begin{description}
		...
		Table \ref{tab:mult_yearWise} shows the number of cases in the sample in which there was a multiplicity of proceedings. As a percentage of the total, the proportion of such cases is small. Note that we expect cases with a multiplicity of proceedings to take more days and more hearings to dispose of. %However, as \Cref{tab:state_multiplicity} shows, the burden caused due to multiplicity varies across states.
	...
	%While hazard models, on the lines of \textcite{datta2017_itatDelays}, would be an ideal choice to use in such an analysis, we chose the fixed-effects model due to the peculiarities of our data. Case characteristics for pending cases cannot be reliably detected because orders for those cases are not available in most instances. Further, given that most cases in our data-set are disposed of, we did not expect significantly different results from using a fixed-effects model. Finally, a fixed-effects model gives coefficients that can be easily interpreted, whereas hazard models yield probabilities for a case to conclude after a given duration. The coefficient from the fixed-effects model is, therefore, easier to interpret for policy reform than the results of a hazard model.
	...
	%For each performance measure, we estimate the following fixed effect regression model:
...
\end{table}
	...
	%The results, shown in Table \ref{tab:disposal_regression}, indicate that non-appearance of the accused typically adds 265 days to the total case duration. Further, the non-appearance of the accused typically adds 7 hearings to the total hearings required to dispose of a case. Both these findings are in line with what we expected.
	...
	Our results indicate that a case being referred to mediation typically adds 100 days to the total time required to dispose of it (see Table \ref{tab:disposal_regression}, columns (3) and (7)). Further, the case is referred to mediation typically adds 3.2 hearings to the number of hearings required to dispose of the case. These findings run counter to our belief and the hypotheses that cases referred to mediation take less time and fewer hearings to dispose of. %On the contrary, they will have the opposite effect. 
	...
	To explore the issue further, Table \ref{tab:mediation} summarises the outcomes of cases referred to mediation. %If we count all instances where a case is settled, withdrawn, or compounded as a successful settlement, then these constitute 78.6\% of cases being referred to mediation. 
	...
	While courts may not be able to reduce the number of hearings required to dispose of a contested case, prioritization and scheduling with minimal delays reflect a positive framework to ensure expeditious trial. %However, the proportion of contested cases reduced between 2014 and 2018. Uncontested cases, that form the majority, require less intervention by courts. They are listed after larger gaps. This may be due to parties needing more time to reach a mutually acceptable decision or courts prioritizing contested cases. Either case suggests there may be some scope for improving the scheduling mechanism for uncontested cases.
	...
	%To test the robustness of our findings, we do two additional checks. First, in Appendix \ref{sec:survivalModel}, we check if our results are consistent across time and States. Second, we run the hazard models considering both disposed and pending cases to reconfirm the effect of each case's characteristics.
	...
	%Furthermore, this paper examines the determinants of case disposition time. 
	...
	%Later, we attempt to quantify the effects of different characteristics of \gls{ni} cases on the case duration.   
	\begin{table}[!ht]
	...
	%	The Amici Curiae and Supreme Court have given the following recommendations to reduce delays and pendency of cheque-dishonor cases:
	%	\begin{enumerate}
		...
		%		\item High Courts should prepare schemes to dispose of cases pending for 2-4, 4-6, 6+ years
		...
		%		\item Explore setting up specialized courts
		%		\item Judicially settle the position of \S~202 CrPC
		%		\item Jurisdictional issues
		%	\end{enumerate}
	
	%At the same time, we find that contested cases take fewer days to dispose of even though they require more hearings. This indicates that judges might be more efficient at resolving these cases than mediation forums. As a corollary, increasing the number of special \gls{ni} courts might have the intended effect of increasing disposal rates and reducing pendency.
	...
	%	The results arising from such analyses will enable policymakers to set up cost-effective delay-reducing measures. 
	...
	\begin{appendices}
		...
		% For help: http://www.sthda.com/english/wiki/cox-proportional-hazards-model#
